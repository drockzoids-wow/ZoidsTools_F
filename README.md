...
The quest inventory contains 6,090 unique IDs after the September 25, 2026 refresh: the current Wowhead Forever listings, retained earlier records, three verified Zephras Isle encounters, and 10 additional quests that 60.tools explicitly lists in the Forever client (with inherited Classic details). This adds 758 quests and includes all 17 discoveries found in the checked character saves. Existing character observations and completion remain separate. Approximate observed NPC positions remain tooltip information, not exact map pins.

The footer shows matching quests separately from the total inventory. "Mine" filters by faction and class; status, area and search also affect the matching count. Datamined entries do not guarantee live availability. Earlier records absent from the latest index and entries with inherited Classic details are identified in tooltips.

Collection evidence and limitations are in `Completionist/Sources.json`; the offline refresh workflow is documented in `Tools/QUEST_DATA.md`. Wowhead's filtered lists supplied 5,196 unique IDs while its headline reported 5,193. Every terminal partition was untruncated and every ID on the unfiltered first page was included, but this discrepancy prevents an exact source-total completeness claim. No addon network requests are made.

Validate release inputs without creating a ZIP: `python Tools/package.py --tag v0.2.8-beta --validate-only`.
