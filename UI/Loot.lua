...
function UI.CreateLootPage(parent)
    ...
    local enabled = UI.CreateCheckbox(page, L["Fast loot"], L["Speed up looting when Auto Loot is active."],
        function() return ns:GetFastLootEnabled() end,
        function(value) ns:SetFastLootEnabled(value) end)
    enabled:SetPoint("TOPLEFT", 0, 0)
    local delay = UI.CreateSlider(page, L["Delay between items"], L["Drag or use the mouse wheel to adjust by 1 millisecond."], 0, 200, 1,
        function() return ns:GetFastLootDelay() * 1000 end,
        function(value) ns:SetFastLootDelay(value / 1000) end, 300,
        function(value) return string.format(L["%.3f s (%d ms)"], value / 1000, math.floor(value + 0.5)) end)
